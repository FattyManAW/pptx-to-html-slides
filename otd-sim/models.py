...
class Scheduler:
    ...
    def sort(self, orders: list[Order], now: datetime | None = None) -> list[Order]:
        ...
        if self.rule == "FIFO":
            ...
            # product_type → cycle_time_hrs lookup (from factory.json)
            _cycle_map = {"A": 4.0, "B": 2.5, "C": 6.0}
            return sorted(orders, key=lambda o: (
                _cycle_map.get(getattr(o, "product_type", "A"), 4.0),
                o.arrival_day
            ))
        elif self.rule == "CR":
...
class SimulationResult:
    ...
    otd_count: int = 0
    warehouse_shipments: int = 0
    avg_yield_rate: float = 0.0
...
class SimulationEngine:
    ...
    def _compute_metrics(self, orders: list[Order],
                         ...
                         dispatch_loop=None) -> dict[str, float]:
        """v0.3: OTD rate + avg_lead_time_days + avg_wip + bottleneck + defects."""
        shipped = [o for o in orders if o.status == OrderStatus.SHIPPED]
        on_time = [o for o in shipped if o.on_time] if shipped else []
        otd = len(on_time) / len(shipped) if shipped else 0.0

        # avg_lead_time_days: from dispatch loop warehouse shipments
        avg_lead = 0.0
        if dispatch_loop and hasattr(dispatch_loop, '_warehouse_shipments'):
            lead_times = [r.get("completed_day", 0) for r in dispatch_loop._warehouse_shipments]
            avg_lead = sum(lead_times) / len(lead_times) if lead_times else 0.0

        # avg_wip + bottleneck + defects from dispatch loop
        avg_wip = 0.0
        bottleneck = ""
        total_def = 0
        if dispatch_loop:
            avg_wip = dispatch_loop.avg_wip
            bottleneck = dispatch_loop.bottleneck_station
            total_def = dispatch_loop.total_defects
        ...
        return {
            ...
            "warehouse_shipments": len(getattr(dispatch_loop, '_warehouse_shipments', [])),
        }
...
#   [x] models.py _compute_metrics 接入 dispatch_loop（avg_wip/bottleneck/defects/lead_time）
#   [x] _schedule_orders: 根據 factory.json product route 分配 station_ids
#   [x] _forward_to_next_station: per-product route callback → StationDispatchLoop
#   [x] Warehouse FIFO queue: 每日出貨 + transit_delay 計算
#   [x] _compute_metrics: avg_lead_time_days + avg_wip + bottleneck + warehouse_shipments
#
...
#   [ ] _ship_orders: VariantParams.transit_delay() 更細粒度
#
# 檔案：showcase/otd-sim/models.py (463 行) + station_dispatch.py (480 行)
